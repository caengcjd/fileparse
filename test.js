...
app.post('/parse', upload.single("file"), function (req, res, next) {
...
var req = http.request(options, function (res_inner) {  
	...
	res_inner.on('end',function(){  
		...
		if(meta.code){
			 restler.post("http://localhost:8000/"+path, {
				...
				res.send({data:meta.data,result:1});return;
			});
			
		}else{
			res.send({data:"文档转换失败!",result:1});return ;
		}
	...
	req.on('error', function (e) { 
		res.send({data:e.message,result:0});return;
	});  
